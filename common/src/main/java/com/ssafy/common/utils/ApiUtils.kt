...
import com.ssafy.common.model.ErrorResponse
...
object ApiUtils {
    ...
    const val PIN_ATTEMPTS_EXCEEDED_MESSAGE =
        "PIN authentication failed 5 times. Please login again."
        ...
        return try {
            ...
            if (response.isSuccessful) {
                ...
                when (errorResponse.getErrorCodeOrDefault()) {
                    ...
                    "PIN_ATTEMPTS_EXCEEDED" -> Result.failure(
                        PinAttemptsExceeded(
                            PIN_ATTEMPTS_EXCEEDED_MESSAGE
                        )
                    )
